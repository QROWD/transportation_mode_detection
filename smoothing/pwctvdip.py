...
def pwc_tvdip(y, lamb=[1.0], display=True, stoptol=1e-3, maxiter=60,
              full=False):
    """
    Performs discrete total variation smoothing (TVD) using a primal-
    dualinterior-point solver. It minimizes the following discrete functional:
    ...
    :param y: Original signal to denoise, size N x 1.
    :param lamb: A vector of positive regularization parameters, size L x 1.
    ...
    :param stoptol: (Optional) Precision as determined by duality gap
    tolerance, if not specified, defaults to 1e-3.
    :param maxiter: (Optional) Maximum interior-point iterations, if not
    specified defaults to 60.
    :param full:
    :return: x, E, s, lambdamax where x is the denoised output signal for each
    value of lambda, size N x L; E is the objective functional at minimum for
    each lambda, size L x 1; s is the optimization result, 1 = solved,
    0 = maximum iterations exceeded before reaching duality gap tolerance,
    size L x 1; and lambdamax is the maximum value of lambda for the given y.
    If lambda >= lambdamax, the output is the trivial constant solution
    x = mean(y).
    """
    y = np.array(y)
    ...
    for l in range(L):
        ...
        if display:
            print('Solving for lambda={0}, lambda/lambda_max={1}\n'
                  'Iter# Primal    Dual      Gap'.format(
                    lamb[l], lamb[l] / lambdamax))
        ...
        for iters in range(maxiter):
            ...
            pobj1 = 0.5 * w.dot(splin.spsolve(DDT, w)) + lamb[l] * np.sum(
                mu1 + mu2)
            ...
            resDual = rz
            resCent = resCent = np.concatenate(
                (-mu1 * f1 - 1 / t, -mu2 * f2 - 1 / t))
