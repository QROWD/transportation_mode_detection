...
def pwc_cluster(y, K=None, soft=False, beta=0, biased=False, display=True,
                stoptol=1e-5, maxiter=50):
    """
    Performs PWC smoothing of the input signal using hard or soft mean-shift,
    ...
    :param y: Original signal to denoise of length N.
    :param K: Number of PWC levels (clusters). If K<N, performs K-means
    clustering. Choose K=[] or K=N to perform mean-shift.
    :param soft: Set this to True to use the soft Gaussian kernel, else uses
    the hard kernel.
    :param beta: Kernel parameter. If soft Gaussian kernel, then this is the
    precision parameter. If hard kernel, this is the kernel support.
    :param biased: Set this to True to use 'biased' mode: that is, the weighted
    mean of the input samples, rather than the current estimated PWC signal.
    Note that if performing K-means, this is automatically True.
    :param display: (Optional) Set to False to turn off progress display, True
    to turn on. If not specifed, defaults to progress display on.
    :param stoptol: (Optional) Precision of estimate as determined by square
    magnitude of the change in the solution. If not specified, defaults to 1e-5.
    :param maxiter: (Optional) Maximum number of iterations. If not specified,
    defaults to 50.
    :return: Denoised output signal.
    """
    y = np.array(y)
    N = len(y)
