...
def pwc_bilateral(
        y, soft=True, beta=200.0, width=5, display=True, stoptol=1e-3,
        maxiter=50):
    """
    Performs PWC smoothing of the input signal using hard or soft kernel
    bilateral filtering.
    Usage:
    ...
    :param soft: Set this to 1 to use the soft Gaussian kernel, else uses the
    hard kernel.
    ...
    :param width: Spatial kernel width W.
    ...
    magnitude of the change in the solution. If not specified, defaults to 1e-3.
    ...
    while iiter < maxiter:
        ...
        if (soft):
            ...
            W = np.zeros((N, N))
            W[np.where(d <= beta ** 2)] = w[
                np.where(d <= beta ** 2)]  # Characteristic (hard) kernel
