...
def pwc_jumppenalty(
        y, square=True, gamma=1.0, display=True, maxiter=50, full=False):
    """
    Performs PWC smoothing of the input signal using least-squares or
    least-absolute (robust) forward jump penalization. It attempts to minimize
    ...
    :param square: Set to 1 to perform least-squares fitting (p=2 in the global
    functional), or 0 to perform least-absolute (robust) fitting (p=1).
    :param gamma: Positive regularization parameter.
    ...
    defaults to 50.
    :param full:
    :return:x and H where x is the denoised output signal, and H is the global
    penalized functional value given the output signal.
    """
    ...
    while iiter < maxiter:
        ...
        for i in range(N):
            ...
            xtest = calc_solution(y, r1, square)
            # Compute likelihood
            if square:
