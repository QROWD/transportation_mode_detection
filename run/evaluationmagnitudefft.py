...
def leave_one_user_out_training(training_data_file_path):
    ...
    for window_size in [15, 30, 45, 60, 90, 120, 150, 180, 210, 240, 270, 300]:
        ...
        results_file_name = f'w{window_size}_s{step_size}_magnitude_fft.csv'
        ...
        with open(results_file_name, 'w') as res_file:
            ...
            for user in data_loader.users:
                ...
                for classifier_name, classifier in classifiers.items():
                    ...
                    probabilities = classifier.predict_proba(test_windows)
                    entropies = map(entropy, probabilities)

                    avg_window_entropy = np.average([e for e in entropies])
                    ...
                    print(f'o_!=:  {not_same}')
                    print(f'o_acc: {acc}')
                    print(f'avg entropy: {avg_window_entropy}')
                    ...
                    csv_writer.writerow([
                        ...
                        avg_window_entropy, same, not_same, acc, smoothed_same,
